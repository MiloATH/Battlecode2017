                        ...
                        if (b.getType() == RobotType.ARCHON) {//TODO: only broadcast once
                            ...
                            if (!stayInPlace && !rc.hasMoved()) {
                                ...
                                if (nextTree != null) {//If still null then there aren't any trees found by treeHideNavigateTo()
                                    ...
                                    distance = rc.getType().strideRadius>distance ? distance : rc.getType().strideRadius;
                                    MapLocation ideal = avoidBulletsMove(opponent, distance);
                                    if (ideal != null && ideal.distanceTo(b.getLocation())<=rc.getLocation().distanceTo(b.getLocation())) {
                                        rc.move(ideal);
                                        //rc.setIndicatorLine(rc.getLocation(),ideal, 255, 0,0);
                                        debug_println("Moving towards opponent and avoiding bullets: " + ideal.toString());
                                    } else {
                                        debug_println("No move alternative that avoids bullets");
                                    }
